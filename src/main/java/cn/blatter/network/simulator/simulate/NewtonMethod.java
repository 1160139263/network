...
public class NewtonMethod {
    ...
    private double[][] getPressure (SimpleMatrix unknown) {
        ...
        for (int i = 0; i < nodeCount; i++) {
            if (network.getNodes().get(i).isPressureState()) {
                pressure[i][0] = network.getNodes().get(i).getPressure();
            }
            else {
    ...
    private double[][] initPressure () {
        ...
        for (int i = 0; i < network.getNodes().size(); i++) {
            if (network.getNodes().get(i).isPressureState()) {
                averagePressure += network.getNodes().get(i).getPressure();
                count ++;
            }
        }
    ...
    private void setResult(SimpleMatrix unknownPressure) {
        ...
        for (int i = 0; i < network.getNodes().size(); i++) {
            if (!network.getNodes().get(i).isPressureState()) {
                network.getNodes().get(i).setPressure(unknownPressure.get(flag, 0));
                flag ++;
            }
        }
        //流量
